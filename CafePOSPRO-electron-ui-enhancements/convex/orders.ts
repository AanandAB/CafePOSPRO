...
export const completeOrder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Update items to "served" status when order is completed
    const updatedItems = order.items.map((item) => ({
      ...item,
    ...
    await ctx.db.patch(args.orderId, {
      ...
      finalAmount,
      items: updatedItems, // Update items with served status
    });
...
export const clearTableOrders = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Delete all orders for this table (instead of just marking as cancelled)
    ...
    return {
      ...
      tableStatus: updatedTable?.status || "unknown",
    };
  },
});
